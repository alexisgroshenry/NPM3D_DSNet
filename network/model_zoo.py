...
class PolarBaseClass(nn.Module):
    ...
    def calc_sem_label(self, sem_logits, inputs, need_add_one=True):
        # extract predicted labels and put in grid format
    ...
    def calc_sem_label_point_logits(self, sem_logits, inputs, need_add_one=True):
        # not clear border effects everywhere --> not used anywhere in the code
...
class PolarOffsetSpconvPytorchMeanshift(PolarOffsetSpconv):
    ...
    def forward(self, batch, is_test=False, before_merge_evaluator=None, after_merge_evaluator=None, require_cluster=True, require_merge=True):
        if self.is_fix_semantic_instance:
            with torch.no_grad():
                # PointNet computes the features from the geometric features
                coor, feature_3d = self.voxelize_spconv(batch)
                # Spconv_salsaNet_res_cfg computes semantic & instance input features (4 * init_size) : it returns twice up0e, hence sem_fea==ins_fea
                sem_fea, ins_fea = self.backbone(feature_3d, coor, len(batch['grid']))
                # Spconv_sem_logits_head_cfg predicts a confidence array (N_classes = 20)
                sem_logits = self.sem_head(sem_fea)
                # Spconv_ins_offset_concatxyz_threelayers_head_cfg predicts the offsets for each grid cell (3) & features (init_size) for further prediction of the instances IDs
        ...
        # DS module to predict the instance IDs : valid is used to mask non instance objects (if is_test, valid is recalculated cf supra)
