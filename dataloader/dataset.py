...
class spherical_dataset(data.Dataset):
  ...
  def __getitem__(self, index):
        'Generates one sample of data'
        data = self.point_cloud_dataset[index] # data corresponds to 1 frame
        if len(data) == 2:
            xyz,labels = data
        elif len(data) == 3:
            xyz,labels,sig = data
        ...
        max_bound_r = np.percentile(xyz_pol[:,0],100,axis = 0) # max rho
        min_bound_r = np.percentile(xyz_pol[:,0],0,axis = 0) # min rho
        max_bound = np.max(xyz_pol[:,1:],axis = 0) # max phi & z
        min_bound = np.min(xyz_pol[:,1:],axis = 0) # min phi & z
        max_bound = np.concatenate(([max_bound_r],max_bound)) # max & min arrays (shape 3)
        min_bound = np.concatenate(([min_bound_r],min_bound)) # max & min arrays (shape 3)
        if self.fixed_volume_space:
            max_bound = np.asarray(self.max_volume_space)
            min_bound = np.asarray(self.min_volume_space)
        ...
        label_voxel_pair = label_voxel_pair[np.lexsort((grid_ind[:,0],grid_ind[:,1],grid_ind[:,2])),:]
        processed_label = nb_process_label(np.copy(processed_label),label_voxel_pair)
        # majority vote inside each voxel for the label
        data_tuple = (voxel_position,processed_label)
...
def collate_fn_BEV(data): # stack alone batch dimension
    ...
    xyz = [d[4] for d in data]                                   # point-wise coor --> cooresponds to return_fea : [relative polar coor(3), polar coor(3), cat coor(2), ref signal(1)]
